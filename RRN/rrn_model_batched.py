...
class RelationUpdateSubject(nn.Module):
    ...
    def forward(
        ...
        #                        ==  e_s * (e_o \cdot w)
        dot_product_ow = torch.sum(
            object_embeddings * self.w.unsqueeze(0), dim=1, keepdim=True
        )
        interaction_term = subject_embeddings * dot_product_ow

        # Update direction
        direction = F.relu(
            self.W_s(subject_embeddings)
            + self.W_o(object_embeddings)
            + interaction_term
        )
...
class ClassUpdate(nn.Module):
    ...
    def forward(
        ...
        concatenation = torch.cat(
            (individual_embeddings, individual_memberships), dim=1
        )
...
class RRN(nn.Module):
    ...
    def forward(
        ...
        memberships_tensor = torch.as_tensor(memberships, dtype=torch.float32).to(
            device
        )
        ...
        for _t in range(self._iterations):
            ...
            for layer_idx, data in grouped_triples.items():
                ...
                # Create long tensors
                s_idx_tensor = torch.tensor(
                    s_idx_batch, dtype=torch.long, device=device
                ).unsqueeze(1)
                o_idx_tensor = torch.tensor(
                    o_idx_batch, dtype=torch.long, device=device
                ).unsqueeze(1)
                ...
                update_term_s_batch, update_term_o_batch = self.layers[layer_idx](
                    e_s_batch, e_o_batch
                )

                # Add updates to the accumulator matrix
                # using scatter_add to add each update term to the correct entity index.
                update_accumulator.scatter_add_(
                    0, s_idx_tensor.expand_as(update_term_s_batch), update_term_s_batch
                )
                update_accumulator.scatter_add_(
                    0, o_idx_tensor.expand_as(update_term_o_batch), update_term_o_batch
                )
            ...
            new_embedding_m = F.normalize(
                updated_embedding_m + update_accumulator, p=2, dim=1
            )
...
class ClassesMLP(nn.Module):
    ...
    def __init__(
        ...
        hidden_size = (
            (embedding_size + classes_count) // 2
...
class RelationMLP(nn.Module):
    ...
    def __init__(
        ...
        # TODO tune hidden layer size
        hidden_size = (
            (2 * embedding_size + 1) // 2 if hidden_size is None else hidden_size
        )
    ...
    def forward(
        self, s_embedding: torch.Tensor, o_embedding: torch.Tensor
    ) -> torch.Tensor:
        """
        Args:
