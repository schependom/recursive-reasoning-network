...
def check_convergence(
    current_loss: float, previous_loss: float, threshold: float = 0.01
) -> bool:
    """
...
def train(
    ...
    # Load knowledge graphs from data directory
    print(f"Using {workers} loader worker(s)")
    kg_list = load_knowledge_graphs(data_dir, num_workers=workers)
    ...
    print(
        f"Initializing RRN with {len(sample_kg.classes)} classes and "
    ...
    while epoch <= max_epochs:
        ...
        for mlp_idx, mlp in enumerate(mlps):
            save_checkpoint(
                mlp, f"{child_checkpoint_path}/mlp{mlp_idx}_epoch-{epoch}.pth"
            )
