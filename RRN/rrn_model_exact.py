    ...
    def forward(
        ...
        if subject_embedding.dim() == 1:
            ...
            dot_product_ow = torch.sum(
                object_embedding * self.w.unsqueeze(0), dim=1, keepdim=True
            )
            interaction_term = subject_embedding * dot_product_ow

        # Update direction
        direction = F.relu(
            self.W_s(subject_embedding) + self.W_o(object_embedding) + interaction_term
        )
...
class RelationUpdateObject(nn.Module):
    ...
    def forward(
        ...
        if object_embedding.dim() == 1:
            ...
            dot_product_sw = torch.sum(
                subject_embedding * self.w.unsqueeze(0), dim=1, keepdim=True
            )
            interaction_term = object_embedding * dot_product_sw
